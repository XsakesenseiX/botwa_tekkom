...
async function triggerHumanHandover(userId, originalMessage, context = "Customer Service") {
    ...
    const currentDayWIB = now.getDay();
    const workingHoursStart = 8;
    const workingHoursEnd = 20;
    const isWorkingDay = currentDayWIB >= 1 && currentDayWIB <= 5;
...
client.on('message', async (message) => {
    try {
        ...
        // --- Global Keyword Handler for "Brosur" ---
        if (userChoiceLower === 'brosur' || userChoiceLower === 'brosus') {
            ...
            userState.set(userId, 'awaiting_role_at_start'); // Arahkan ke menu utama setelah info brosur
            // Reset inactivity timer because this is an interaction
            if (userActivityTimers.has(userId)) clearTimeout(userActivityTimers.get(userId));
            const activityTimerIdBrosur = setTimeout(async () => { /* ... (sama seperti timer utama) ... */ }, INACTIVITY_TIMEOUT_MS);
            userActivityTimers.set(userId, activityTimerIdBrosur);
            return; // Hentikan pemrosesan lebih lanjut untuk pesan ini
        }
        
        // --- Inactivity Timer Reset (setelah cek keyword global) ---
        if (userActivityTimers.has(userId)) clearTimeout(userActivityTimers.get(userId));
        ...
        if (!currentState || currentState === 'awaiting_role_at_start') {
            ...
            // ... (logika mahasiswa_menu tetap sama)
            if (userChoice === '0') {
                userState.set(userId, 'awaiting_role_at_start');
            ...
            // ... (logika layanan_prodi_mahasiswa_menu tetap sama)
            let replyMessage = "";
            if (userChoice === '0') {
                userState.set(userId, 'mahasiswa_menu');
                await message.reply(menuData.mahasiswaMenu);
            ...
            // ... (logika awaiting_dosen_wali_input tetap sama)
            ...
            if (foundDosenEntries.length === 1) {
                ...
                await message.reply(replyMsg + "\n\n" + menuData.askForNpmOrName); // Minta input lagi
                userState.set(userId, 'awaiting_dosen_wali_input'); // Tetap di state ini
                userState.set(userId + '_return_to_after_dosenwali', returnToMenu); // Set ulang return path
            } else {
            ...
            // ... (logika calon_mahasiswa_menu tetap sama)
             if (userChoice === '0') { 
                userState.set(userId, 'awaiting_role_at_start');
                await message.reply(menuData.roleSelectionQuestion);
            } else if (userChoice === '1') { 
            ...
            // ... (logika info_layanan_prodi_umum_submenu tetap sama)
            if (userChoice === '0') { 
                userState.set(userId, 'calon_mahasiswa_menu');
                await message.reply(menuData.calonMahasiswaMenu);
            } else if (userChoice === '1') { 
